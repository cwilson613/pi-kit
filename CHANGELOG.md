...
- Added the compiled Slice-1 durable session-authority substrate with strict versioned prompt, FIFO queue, turn, interruption, minimum invocation, and terminal facts; contiguous replay and identity invariants; synced adjacent JSONL plus replaceable snapshot caches; and deterministic idempotent runtime-loss closure. The interactive runtime now uses one compiled frontend-neutral prompt, turn, and supervisor implementation while durable ingress ownership remains gated on the Slice-1 integration lane.
