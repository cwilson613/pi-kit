...
- [ ] 3.7 Route tools, actions, extensions, host effects, and privileged internal calls through the same lease path while retaining direct pure/read-only service calls. In progress: admission and lease validation are kind-aware; graph-registered TUI, CLI, and ACP feature commands use explicit operator scopes and leased dispatch; non-TUI control forwarding retains its surface; model-loop path grants, automatic memory ingestion, and host-mediated persona/tone switches use internal leases. Remaining work includes Web/IPC command declaration support, truthful service declarations for managed delegation and voice effects, typed read-only context access, extension polling RPC, and nested extension/MCP HostActions.
