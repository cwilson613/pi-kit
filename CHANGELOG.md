...
- Added an EventBus-owned managed-service sidecar for Slice 6.1.6. Synthetic managed candidates now publish through one shared admission-table swap, transfer exact unchanged generations without cleanup, retain bounded rejected-candidate cleanup, drain replaced generations from the publication instant, and expose cancellation-safe managed shutdown while releasing fully settled retired implementations. Feature/contribution-graph declarations, lifecycle diagnostics, host shutdown wiring, and production-domain migration remain deferred.
