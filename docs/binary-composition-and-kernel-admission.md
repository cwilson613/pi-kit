...
Model-tool dispatch now crosses one kernel-owned invocation service before host delegation or local owner execution. The service resolves the invocation against the accepted graph, combines the existing name-derived RBAC ceiling with layered permission policy and operator approval, and issues a call-, principal-, scope-, capability-, owner-, effect-, transition-, and composition-generation-bound lease. EventBus revalidates the accepted generation and owner immediately before execution; stale, mismatched, closed, reused, unknown, or incompletely declared calls receive no executable authority. Lease claim and terminal closure are exactly once, and rejected candidate publications leave leases for the retained generation valid.

This first invocation lane is intentionally ephemeral. It does not persist `Prepared`, `Dispatched`, acknowledgement, settlement, or unknown-completion facts and therefore makes no crash-consistency or retry-safety claim. Declaration-derived authority metadata, durable state, mutation fencing, and migration of direct operator, extension, host-effect, and privileged internal compatibility paths remain later Slice-3 work. Reactive path grants and extension HostAction approval can only narrow the upstream lease decision.
