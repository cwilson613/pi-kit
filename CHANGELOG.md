...
- Generalized invocation admission and generation revalidation beyond model tools. Graph-registered TUI, CLI, and ACP feature commands now execute under explicit operator leases, and model-loop path grants use authority-backed internal leases with acknowledgement and settlement before results return; unresolved service, control-runtime, extension-RPC, and nested host-effect paths remain explicit compatibility work.
