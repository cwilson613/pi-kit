...
Slice 3.7 begins by making admission and dispatch kind-aware rather than tool-only. Graph-registered feature commands invoked through TUI, CLI remote execution, or ACP now carry explicit operator principals and declared surfaces through generation revalidation, acknowledgement, settlement, and exactly-once lease closure. Model-loop path grants invoke the graph-declared `trust_directory` internal owner under an internal principal while retaining the parent session and turn authority. These migrations do not fabricate durability for idle operator calls: without an authority turn they remain explicit ephemeral leases. Control-runtime bridges that have already lost ingress provenance, direct service-triggered tools, arbitrary extension polling RPC, and nested extension/MCP HostActions remain compatibility paths until they gain truthful caller/surface declarations and owner-bound dispatch contracts.
