...
pub async fn execute_streaming(
    ...
    // Command-aware output compression — domain-specific patterns strip
    // noise (progress bars, repeated compile lines, boilerplate) while
    // preserving actionable content (errors, warnings, summaries).
    let compressed = compress_command_output(command, &clean_output);
...
/// Command-aware output compression. Applies domain-specific patterns to
/// strip noise while preserving actionable content. Inspired by PandaFilter's
/// handler patterns but implemented as simple inline rules — no external deps.
///
/// Returns the compressed output, or the original if no patterns matched.
fn compress_command_output(command: &str, output: &str) -> String {
    ...
    // ── Cargo test — keep summary, collapse individual test lines ──────
    if cmd.starts_with("cargo test") {
        ...
        if test_count > 5 {
            ...
            if pass_count > 0 {
                compressed.push(format!("[{pass_count} tests passed]"));
            }
            compressed.extend(fail_lines.iter().map(|s| s.to_string()));
            compressed.extend(result);
            return compressed.join("\n");
        }
    }

    // ── npm/yarn/pnpm install — strip progress, keep summary ───────────
    ...
    // ── pip install — strip download progress, keep result ──────────────
    ...
    // ── Generic progress bar / spinner stripping ───────────────────────
    // Collapse lines that are purely progress indicators
    let progress_chars = ['█', '▓', '▒', '░', '━', '─', '⣿', '⠿', '⠋', '⠙', '⠹', '⠸'];
    ...
    if progress_count > lines.len() / 3 {
        ...
        if filtered.len() < lines.len() {
            ...
            result.push(&"");
            let msg = format!("[{stripped} progress lines stripped]");
            // Can't return a reference to local, rebuild
    ...
    // No patterns matched — return as-is
    output.to_string()
}
...
mod tests {
    ...
    #[test]
    fn compress_cargo_build_collapses_compiling_lines() {
        let output = (0..20)
            .map(|i| format!("   Compiling crate-{i} v0.1.0"))
            .chain(std::iter::once("    Finished `dev` profile in 4.2s".to_string()))
            .collect::<Vec<_>>()
            .join("\n");
        let compressed = compress_command_output("cargo build", &output);
        assert!(compressed.contains("[compiled 20 crates]"), "{compressed}");
        assert!(compressed.contains("Finished"), "{compressed}");
        assert!(
            compressed.lines().count() < 5,
            "should be compact: {compressed}"
        );
    }

    #[test]
    fn compress_cargo_test_keeps_failures() {
        ...
        let compressed = compress_command_output("cargo test", &output);
        assert!(compressed.contains("12 tests passed"), "{compressed}");
        assert!(compressed.contains("test_bad"), "{compressed}");
        assert!(compressed.contains("FAILED"), "{compressed}");
    }

    #[test]
    fn compress_pip_strips_download_progress() {
        let mut lines = Vec::new();
        lines.push("Collecting requests".to_string());
        for i in 0..15 {
            lines.push(format!(
                "  Downloading requests-{i}.whl (58 kB)\n     ━━━━━━━━━━ 58/58 kB"
            ));
        }
        lines.push("Successfully installed requests-2.31.0".to_string());
        let output = lines.join("\n");
        let compressed = compress_command_output("pip install requests", &output);
        assert!(compressed.contains("Successfully installed"), "{compressed}");
        assert!(!compressed.contains("━"), "{compressed}");
    }

    #[test]
    fn compress_short_output_unchanged() {
        let output = "line1\nline2\nline3";
        let compressed = compress_command_output("ls -la", output);
