...
///
/// The prompt is deliberately part of the skills domain crate so upstream
/// adaptation guidance, manifest schema, and provenance linting evolve together.
...
7. Include structured provenance in the generated YAML frontmatter whenever the skill derives from upstream material:

```yaml
provenance:
  source_url: <canonical upstream URL>
  source_ref: <commit, tag, branch, release, or catalog snapshot>
  source_path: <path within upstream source>
  retrieved_at: <date or timestamp if known>
  adaptation: exact_import | trusted_executable_import | prompt_only_adaptation | clean_room_adaptation
  omitted_assets: [<upstream assets deliberately omitted>]
  trusted_assets: [<upstream executable assets explicitly trusted/imported>]
```

Also include readable `## Provenance`, `## Adaptation Notes`, and `## Safety Notes` sections in the body so operators can review the adaptation without parsing frontmatter.
