...
- Generalized invocation admission and generation revalidation beyond model tools. Graph-registered TUI, CLI, ACP, Web, and IPC feature commands now execute under explicit operator leases and owner-declared surfaces; model-loop path grants, automatic memory ingestion, and host-mediated persona/tone switches use internal leases; managed-delegation tools admit explicit service principals and Web/Daemon surfaces instead of dispatching directly. Memory mutation and delegate-result tools now declare state-changing effects, and operator context-pack reads use a typed read-only service instead of tool dispatch; the extension-provided voice stop, extension RPC, and nested host effects remain explicit compatibility work.
