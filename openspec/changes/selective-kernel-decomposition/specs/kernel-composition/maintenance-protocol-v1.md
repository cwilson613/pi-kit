...
`TransactionV1` contains request ID, canonical command fingerprint, domain key, root identities, ordered `steps`, current state, timestamps, and audit sequence. Each step contains `kind`, parent identity, basename digest, optional expected existing target identity, optional expected absence, state, and observed post-state. Transaction infrastructure writes and audit writes are not represented as recursively nested target steps.
