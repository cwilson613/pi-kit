...
Normal composition must remain parity-equivalent across canonical fixtures BP01-BP09 before direct compatibility calls are removed. Their canonical baseline is the direct implementation and assertions at commit `9c3a9860`. Task 6.1.5 must first materialize these cases as shared table-driven vectors that invoke both direct and service paths; expected enum/boolean values and BP09 message strings must be literal fixture data that survives deletion of direct production calls.
