...
- Generalized invocation admission and generation revalidation beyond model tools. Graph-registered TUI, CLI, and ACP feature commands now execute under explicit operator leases; non-TUI control forwarding preserves its surface; model-loop path grants, automatic memory ingestion, and host-mediated persona/tone switches use internal leases with acknowledgement and settlement before results return. Memory mutation tools now declare state-changing effects, while Web/IPC declaration support, managed service and voice calls, typed read-only context access, extension RPC, and nested host effects remain explicit compatibility work.
