...
use crate::features::delegate::{DelegateChildFailureKind, DelegateTask, DelegateTaskStatus};
        ...
        match value {
            ...
            DelegateTaskStatus::Failed { error, kind } => Self::Failed {
                ...
                safe_message: error
                    .lines()
                    .next()
                    .unwrap_or("Delegate worker failed")
                    .to_string(),
            },
    ...
    fn from(task: DelegateTask) -> Self {
        ...
        Self {
            ...
            last_tool: task.last_tool.map(|tool| ToolActivityObservation {
                ...
                args_summary: task
                    .last_tool_activity
                    .and_then(|activity| activity.args_summary),
            }),
            last_turn: task.last_turn,
            checklist: task
                .tasks
                .into_iter()
                .map(|item| ChecklistItemObservation {
...
pub fn validate_identity<T>(request: &Envelope<T>) -> Result<(), String> {
    ...
    if request.managed_run_id.trim().is_empty() || request.worker_id.trim().is_empty() {
        return Err("missing_identity".into());
    }
    Ok(())
}
...
pub fn parse_operation(
    method: &str,
    payload: &Value,
) -> Result<(Envelope<Value>, SupervisorOperation), String> {
    let envelope: Envelope<Value> =
        serde_json::from_value(payload.clone()).map_err(|e| format!("invalid_request:{e}"))?;
    ...
    let operation = match method {
        ...
        "delegate_dispatch" => {
            ...
            if body.request.directive.trim().is_empty()
                || body.request.supervisor_deadline_seconds == 0
            {
                return Err("invalid_dispatch_request".into());
            }
            SupervisorOperation::Execute {
        ...
        "delegate_get" => {
            let body: TaskBody = serde_json::from_value(body.clone())
                .map_err(|e| format!("invalid_task_request:{e}"))?;
            SupervisorOperation::GetObservation {
        ...
        "delegate_result" => {
            ...
            SupervisorOperation::Execute {
                ...
                args: serde_json::json!({"task_id": body.task_id}),
            }
        }
        "delegate_cancel" => {
            let body: CancelBody = serde_json::from_value(body.clone())
                .map_err(|e| format!("invalid_cancel_request:{e}"))?;
            if body
                .reason
                .as_ref()
                .is_some_and(|r| r.len() > MAX_REASON_BYTES)
            {
                return Err("oversized_reason".into());
            }
            SupervisorOperation::Execute {
                tool: crate::tool_registry::delegate::DELEGATE_CANCEL,
                args: serde_json::json!({"task_id": body.task_id, "reason": body.reason}),
            }
        }
...
pub fn response(
    ...
    let text = result
        ...
        .unwrap_or_default();
    if method == "delegate_result" && text.len() > MAX_RESULT_BYTES {
    ...
    let response = match method {
        ...
        "delegate_dispatch" => serde_json::to_value(DelegateDispatchResponse {
            ...
            task_id,
            effective_policy: details
                .get("effective_policy")
                .cloned()
                .map(serde_json::from_value)
                .transpose()
                .map_err(|error| format!("invalid_effective_policy:{error}"))?,
            rejection: None,
        }),
...
mod tests {
    ...
    fn rejects_cross_contract_versions_and_oversized_reasons() {
        ...
        assert!(
            parse_operation("delegate_get", &bad)
                .unwrap_err()
        ...
        assert_eq!(
            parse_operation("delegate_cancel", &bad).unwrap_err(),
            "oversized_reason"
        );
    }
    ...
    fn responses_match_auspex_flat_envelope() {
        ...
        let response =
            observation_response(&envelope, observation(DelegateWireStatus::Running, None));
    ...
    fn envelope() -> Envelope<Value> {
        Envelope {
            schema_version: 1,
            managed_run_id: "11111111-1111-4111-8111-111111111111".into(),
            worker_id: "22222222-2222-4222-8222-222222222222".into(),
            body: Value::Null,
        }
    }
    ...
    fn golden_fixtures_match_typed_serialization() {
        let cases = [
            (
                "01-running.json",
                observation(DelegateWireStatus::Running, None),
            ),
            (
                "02-completed-success.json",
                DelegateObservation {
                    completed_at_unix_ms: Some(2000),
                    ..observation(
                        DelegateWireStatus::Completed { success: true },
                        Some("done"),
                    )
                },
            ),
            (
                "03-completed-unsuccessful.json",
                DelegateObservation {
                    completed_at_unix_ms: Some(2000),
                    ..observation(
                        DelegateWireStatus::Completed { success: false },
                        Some("checks failed"),
                    )
                },
            ),
            (
                "04-typed-failure.json",
                DelegateObservation {
                    completed_at_unix_ms: Some(2000),
                    ..observation(
                        DelegateWireStatus::Failed {
                            failure_kind: FailureKind::ProviderStartup,
                            safe_message: "provider unavailable".into(),
                        },
                        None,
                    )
                },
            ...
            (
                "05-cancel-acknowledged.json",
                observation(
                    DelegateWireStatus::Cancelled {
