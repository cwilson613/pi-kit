...
fn check_github() -> AuthResult {
    ...
    if ok {
        ...
        AuthResult {
            ...
            detail,
            error: None,
            refresh: Some("gh auth login".into()),
        }
    } else {
        ...
        AuthResult {
            ...
            status,
            detail: reason,
            error: Some(output.chars().take(300).collect()),
            refresh: Some("gh auth login".into()),
        }
    }
}
...
fn check_gitlab() -> AuthResult {
    if !has_cmd("glab") {
        if std::env::var("GITLAB_TOKEN").is_ok_and(|v| !v.is_empty()) {
            return AuthResult {
                ...
                status: Status::Ok,
                detail: "GITLAB_TOKEN set (glab CLI not installed)".into(),
                error: None,
                refresh: None,
            };
        }
        return AuthResult {
            provider: "gitlab".into(),
            status: Status::Missing,
            detail: "glab CLI not installed".into(),
            error: None,
            refresh: None,
        };
    }
    ...
    if ok {
        ...
        AuthResult {
            ...
            detail,
            error: None,
            refresh: Some("glab auth login".into()),
        }
    } else {
        let (status, reason) = diagnose_error(&output);
        AuthResult {
...
fn check_aws() -> AuthResult {
    if !has_cmd("aws") {
        return AuthResult {
            provider: "aws".into(),
            status: Status::Missing,
            detail: "aws CLI not installed".into(),
            error: None,
            refresh: None,
        };
    }
    ...
    if ok {
        ...
        AuthResult {
            ...
            detail,
            error: None,
            refresh: Some("aws sso login".into()),
        }
    } else {
        ...
        AuthResult {
            ...
            status,
            detail: reason,
            error: Some(stderr.chars().take(300).collect()),
            refresh: Some("aws sso login".into()),
        }
    }
}
...
fn check_kubernetes() -> AuthResult {
    if !has_cmd("kubectl") {
        return AuthResult {
            ...
            status: Status::Missing,
            detail: "kubectl not installed".into(),
            error: None,
            refresh: None,
        };
    }
    ...
    if !ok {
        return AuthResult {
            provider: "kubernetes".into(),
            status: Status::None,
            detail: "No context set".into(),
            error: None,
            refresh: Some("kubectl config use-context <ctx>".into()),
        };
    }
    ...
    if ok2 {
        ...
        AuthResult {
            provider: "kubernetes".into(),
            status,
            detail: format!("context: {context} — {reason}"),
            error: Some(stderr2.chars().take(300).collect()),
            refresh: None,
        }
    }
}
...
fn check_oci() -> AuthResult {
    let cmd = if has_cmd("podman") {
        "podman"
    } else if has_cmd("docker") {
        "docker"
    } else {
        return AuthResult {
            provider: "oci".into(),
            status: Status::Missing,
            detail: "Neither podman nor docker installed".into(),
            error: None,
            refresh: None,
        };
    };
    ...
    if ok {
        AuthResult {
            provider: "oci".into(),
            status: Status::Ok,
            detail: format!("ghcr.io: {} ({cmd})", stdout.trim()),
            error: None,
            refresh: None,
        }
    } else {
        AuthResult {
            provider: "oci".into(),
            status: Status::None,
            detail: format!("Not logged in to ghcr.io ({cmd})"),
            error: None,
            refresh: Some(format!(
                "gh auth token | {cmd} login ghcr.io -u $(gh api user --jq .login) --password-stdin"
            )),
        }
    }
}
...
fn check_vault() -> AuthResult {
    if !has_cmd("vault") {
        return AuthResult {
            provider: "vault".into(),
            status: Status::Missing,
            detail: "vault CLI not installed".into(),
            error: None,
            refresh: None,
        };
    }
    if std::env::var("VAULT_ADDR").is_err() {
        return AuthResult {
            provider: "vault".into(),
            status: Status::None,
            detail: "VAULT_ADDR not set".into(),
            error: None,
            refresh: Some("vault login".into()),
        };
    }
    ...
    if ok {
        let detail = serde_json::from_str::<Value>(&stdout)
            ...
            .and_then(|v| {
                ...
                let expire = data["expire_time"]
                    .as_str()
                    .map(|e| e.split('T').next().unwrap_or(e))
                    .unwrap_or("no expiry");
