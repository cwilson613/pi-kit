...
- Changed `@` project-file selection from eager byte injection to an explicit artifact reference: text, source, PDF, and other non-image selections now tell the agent which path to inspect with harness tooling, while supported image files remain multimodal attachments. The composer preserves the filename as an `@ filename` token, and the picker now presents concise filenames, parent paths, and the on-demand inspection contract instead of generic “inject” language. Codex HTTP 400 responses are also classified as non-retryable bad requests, preventing malformed attachment payloads from masquerading as upstream 5xx retries.
