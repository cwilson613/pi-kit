...
Invocation admission and lease revalidation are now kind-aware rather than tool-only. Graph-registered feature commands from TUI, CLI remote execution, and ACP enter with explicit operator principals and declared surfaces, then acknowledge, settle, and close through the shared lease lifecycle before returning their result. Model-loop path grants invoke the graph-declared `trust_directory` internal owner under an internal principal while retaining parent session and turn authority. Idle operator commands remain ephemeral because no authority turn exists; the runtime does not fabricate durable scope. Control-runtime bridges that have lost ingress provenance, direct service-triggered tools, arbitrary extension polling RPC, and nested extension/MCP HostActions remain named compatibility paths. Reactive path grants and extension HostAction approval can only narrow an upstream lease decision.
