...
pub fn render_bootstrap(status: &HarnessStatus, color: bool) -> String {
    ...
    for b in &status.inference_backends {
        if b.available {
            let n = b.models.len();
            out.push_str(&format!(
                "  {green}✓{reset} {:<12} {dim}{n} model{}{reset}\n",
                b.name,
                if n == 1 { "" } else { "s" }
            ));
        }
...
mod tests {
    ...
    fn status_command_rerender_no_color() {
        ...
        assert!(
            !output.contains("\x1b["),
            "slash command output must not have ANSI codes"
        );
        assert!(
            output.contains("Systems Engineer"),
