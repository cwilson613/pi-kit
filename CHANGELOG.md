...
- Added the compiled Slice-1 durable session-authority substrate with strict versioned prompt, FIFO queue, turn, interruption, minimum invocation, and terminal facts; contiguous replay and identity invariants; content-addressed attachment staging; synced adjacent JSONL plus replaceable snapshot caches; and deterministic idempotent runtime-loss closure. Interactive, ACP, daemon, Web/IPC, and bounded hosts now route overlapping prompt, queue, cancellation, timeout, and terminal semantics through one frontend-neutral supervisor per session.
