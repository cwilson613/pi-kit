//! Docker Compose export — generate compose service definitions from Sandbox profiles.
//!
//! Ensures Sandbox profiles are deployable via standard tooling (docker-compose,
//! podman-compose, k8s kompose) without vendor lock-in to our spawn path.

use std::fmt::Write;

use super::profile::{SandboxNetworkPolicy, SandboxPortProtocol, SandboxProfile};

/// Generate a docker-compose service YAML fragment from a Sandbox profile.
///
...
pub fn to_compose_yaml(profile: &SandboxProfile, service_name: Option<&str>) -> String {
    ...
    match &caps.network {
            ...
            if !ports.is_empty() {
                ...
                for pm in ports {
                    let proto = match pm.protocol {
                        SandboxPortProtocol::Tcp => "tcp",
                        SandboxPortProtocol::Udp => "udp",
                    };
        ...
        SandboxNetworkPolicy::Custom(net) => {
    ...
    match &caps.network {
        SandboxNetworkPolicy::Isolated => {}
        SandboxNetworkPolicy::Host => {} // host mode exposes all ports
        _ => {
            // Add control port if not already in bridge ports
            let has_control_port = matches!(&caps.network,
                SandboxNetworkPolicy::Bridge { ports } if ports.iter().any(|p| p.container == 7842)
            );
            if !has_control_port {
    ...
    // Egress filter JSON
    if let SandboxNetworkPolicy::Egress { filter: Some(f) } = &caps.network {
...
pub fn to_compose_file(profile: &SandboxProfile, service_name: Option<&str>) -> String {
    let mut out = String::new();
    out.push_str("# Generated by omegon sandbox compose\n");
...
"#;
        ...
        assert!(file.starts_with("# Generated by omegon sandbox compose"));
