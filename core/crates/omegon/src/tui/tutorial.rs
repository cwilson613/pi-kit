...
mod tests {
    ...
    fn auspex_browser_step_exists_in_both_modes() {
        // The tutorial auto-opens Auspex when advancing to the
        // browser step. Verify the step title exists.
        let tut = Tutorial::new();
        let has_dash = tut.steps().iter().any(|s| s.title == "Auspex Browser");
        assert!(has_dash, "hands-on mode must have an Auspex Browser step");

        let demo = Tutorial::new_demo(true);
        let has_dash = demo.steps().iter().any(|s| s.title == "Auspex Browser");
        assert!(has_dash, "demo mode must have an Auspex Browser step");
    }

    #[test]
    fn auspex_browser_step_mentions_dash_as_compatibility_path() {
        let step = STEPS_ORIENTATION
            .iter()
            .find(|s| s.title == "Auspex Browser")
            .expect("orientation mode must include the Auspex browser step");
        assert!(step.body.contains("/dash"));
        assert!(step.body.contains("compatibility path") || step.body.contains("compatibility"));
    }
