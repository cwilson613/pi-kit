//! Sandbox profile registry — built-in domain profiles + user-defined custom profiles.
...
use super::profile::{SandboxCapabilities, SandboxDomain, SandboxProfile, SandboxResourceLimits};

const DEFAULT_REGISTRY: &str = "ghcr.io/styrene-lab";

/// Registry of available Sandbox profiles.
...
impl SandboxRegistry {
    ...
    /// - `~/.config/omegon/sandbox/*.toml` (global)
    /// - `<project_root>/.omegon/sandbox/*.toml` (project-local)
    ...
    pub fn resolve(&self, name_or_hash: &str) -> Option<&SandboxProfile> {
        // Direct name match
    ...
    pub fn resolve_for_domain(&self, domain: &SandboxDomain) -> Option<&SandboxProfile> {
        self.profiles
            .values()
    ...
    pub fn list(&self) -> Vec<&SandboxProfile> {
...
fn builtin_domains() -> Vec<SandboxDomain> {
    vec![
        SandboxDomain::Chat,
        SandboxDomain::Coding,
        SandboxDomain::CodingPython,
        SandboxDomain::CodingNode,
        SandboxDomain::CodingRust,
        SandboxDomain::Infra,
        SandboxDomain::Full,
    ]
}
...
fn load_custom_profiles(
    dir: &Path,
    profiles: &mut HashMap<String, SandboxProfile>,
    builtins: &[String],
) {
    ...
    for entry in entries.flatten() {
        ...
        if path.extension().is_some_and(|ext| ext == "toml") {
            match SandboxManifest::from_file(&path) {
                Ok(manifest) => {
                    ...
                    // could ship .omegon/sandbox/coding.toml to hijack the
                    // built-in coding profile with a custom image.
                    if builtins.contains(&profile.name) {
                        tracing::warn!(
                            name = %profile.name,
                            path = %path.display(),
                            "custom sandbox profile shadows a built-in — ignoring \
                ...
                Err(e) => {
                    tracing::warn!(
                        path = %path.display(),
                        error = %e,
                        "failed to load sandbox profile"
                    );
                }
            }
        }
...
mod tests {
    ...
    fn builtin_profiles_registered() {
        let registry = SandboxRegistry::load(Path::new("/nonexistent"), None).unwrap();
    ...
    fn resolve_by_domain() {
        ...
        let profile = registry
            .resolve_for_domain(&SandboxDomain::CodingRust)
            .unwrap();
