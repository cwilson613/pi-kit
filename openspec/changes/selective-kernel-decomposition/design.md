...
Slice 3.7 begins by making admission and dispatch kind-aware rather than tool-only. Graph-registered feature commands invoked through TUI, CLI remote execution, ACP, Web, or IPC now carry explicit operator principals and owner-declared surfaces through generation revalidation, acknowledgement, settlement, and exactly-once lease closure. Model-loop path grants invoke the graph-declared `trust_directory` internal owner under an internal principal while retaining the parent session and turn authority. Automatic memory ingestion and host-mediated persona/tone switches use explicit internal bindings and leases; model-facing memory mutations now declare state-changing effects instead of read-only orientation. Managed-delegation tools declare both model and service principals plus Model/Web/Daemon surfaces, and authenticated supervisor calls use service leases instead of direct EventBus dispatch; `agents_status` resolves to the owned delegate-status tool rather than an unowned name. Operator context-pack requests now call the shared context provider through a typed read-only service handle rather than impersonating a tool invocation. The extension-provided `voice_session_stop` tool declares model and TUI surfaces plus service authority, and the interactive coordinator invokes it under the promoted turn's real authority. Lease-less imperative `actions/execute` requests now fail closed before host execution, and an operator approval only contributes the operator gate instead of fabricating project, runtime, or trusted-origin authority. These migrations do not fabricate durability for idle or post-loop calls: without an active authority turn they remain explicit ephemeral leases. ACP and vox extension RPC plus parent-bound declarative extension/MCP HostActions remain compatibility paths.
