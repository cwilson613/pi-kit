...
- Completed the Slice-5.5 recovery campaign so every frozen manifest row enters an exhaustive consumer-specific oracle, including projection, host-record, and mirror consumers that were previously skipped by the dispatcher; AC13 now reconstructs a real immutable chunk from a chunk-bearing mixed-lineage fixture, copied authority fixtures preserve restrictive permissions, and default projection output is checked for restricted-continuity leakage. The focused campaign passes on macOS, Ubuntu, and Windows within budget. Public documentation, canonical snippets, configuration, and dual-write closeout remain deferred to Slice 5.6.
