...
- [ ] 3.7 Route tools, actions, extensions, host effects, and privileged internal calls through the same lease path while retaining direct pure/read-only service calls. In progress: admission and lease validation are kind-aware; graph-registered TUI, CLI, and ACP feature commands use explicit operator scopes and leased dispatch; model-loop path grants use authority-backed internal leases. Remaining work includes provenance-preserving control-runtime actions, direct service-triggered tools, extension polling RPC, nested extension/MCP HostActions, and classification of direct pure/read-only service calls.
