...
- Hardened the Slice-5.5 recovery campaign so every frozen manifest row enters a consumer-specific oracle, including projection, host-record, and mirror consumers that were previously skipped by the dispatcher; copied authority fixtures now preserve restrictive permissions and default projection output is checked for restricted-continuity leakage. The final audit keeps Slice 5.5 open because several rows still use shared law probes rather than their exact frozen adapter interaction and Linux/Windows campaign evidence is absent. Public documentation, canonical snippets, configuration, and dual-write closeout remain deferred to Slice 5.6.
