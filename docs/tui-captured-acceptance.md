...
This foundation covers fresh-session startup, terminal input, streaming completion, second submission, resize, denied tool execution, approval visibility above Settings, and return to Settings. It uses four local provider requests and no paid inference. It does not yet cover active-turn cancellation, resume, project navigation, colors, or terminal-emulator portability. Add those scenarios as their shell contracts are implemented. The active reconstruction plan is `openspec/changes/tui-project-shell/`.
