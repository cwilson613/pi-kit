...
use crate::{ChangeState, JsonFileStore, Lifecycle as OpsxLifecycle, OpsxError};
...
pub fn write_archive_tx(
    repo_path: &Path,
    tx: &OpenSpecArchiveTransaction,
) -> Result<(), OpsxError> {
    let path = archive_tx_path(repo_path, &tx.change);
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|error| opsx_store_error("create archive transaction directory", error))?;
    }
    let json = serde_json::to_string_pretty(tx).map_err(|error| {
        OpsxError::StoreError(format!("serialize archive transaction: {error}"))
    })?;
    std::fs::write(&path, json)
        .map_err(|error| opsx_store_error("write archive transaction", error))?;
...
pub fn remove_archive_tx(repo_path: &Path, change: &str) -> Result<(), OpsxError> {
    let path = archive_tx_path(repo_path, change);
    if path.exists() {
        std::fs::remove_file(path)
            .map_err(|error| opsx_store_error("remove archive transaction", error))?;
    }
    Ok(())
}
...
) -> Result<Vec<String>, OpsxError> {
    let tx_dir = archive_tx_dir(repo_path);
    let entries = match std::fs::read_dir(&tx_dir) {
        Ok(entries) => entries,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => return Ok(Vec::new()),
        Err(err) => return Err(opsx_store_error("read archive transactions", err)),
    };

    let mut recovered = Vec::new();
    for entry in entries {
        let path = entry
            .map_err(|error| opsx_store_error("read archive transaction entry", error))?
            .path();
        if path.extension().and_then(|ext| ext.to_str()) != Some("json") {
            continue;
        }
        let content = std::fs::read_to_string(&path)
            .map_err(|error| opsx_store_error("read archive transaction", error))?;
        let tx: OpenSpecArchiveTransaction = serde_json::from_str(&content).map_err(|error| {
            OpsxError::StoreError(format!("parse archive transaction: {error}"))
        })?;
