...
This lane still does not persist acknowledgement or terminal settlement and does not classify new dispatched calls after recovery. Through Slice 3.3, completed calls intentionally remain durably `Dispatched`; Slice 3.4 owns acknowledgement, settlement, and unknown-completion recovery. Mutation fencing and migration of direct operator, extension, host-effect, and privileged internal compatibility paths remain later Slice-3 work. Reactive path grants and extension HostAction approval can only narrow the upstream lease decision.
