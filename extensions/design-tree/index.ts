 ...
 */
export interface DesignSpecBindingResult {
	archived: boolean;
	active: boolean;
	changeName: string | null;
}

export function resolveDesignSpecBinding(cwd: string, node: DesignNode): DesignSpecBindingResult {
	const openspecDir = getOpenSpecDir(cwd);
	if (!openspecDir) return { archived: false, active: false, changeName: null };

	const candidateName = node.openspec_change ?? node.id;

	// Check active changes/
	const changesDir = path.join(openspecDir, "changes");
	if (fs.existsSync(path.join(changesDir, candidateName))) {
		return { archived: false, active: true, changeName: candidateName };
	}

	// Check archive/ (entries are <timestamp>-<changeName>)
	...
	if (fs.existsSync(archiveDir)) {
		for (const entry of fs.readdirSync(archiveDir, { withFileTypes: true })) {
			...
			if (name === candidateName) {
				return { archived: true, active: false, changeName: candidateName };
			}
		}
	}

	return { archived: false, active: false, changeName: candidateName };
}
				...
				case "ready": {
					...
					const readyNodes = Array.from(tree.nodes.values())
						.filter((n) => {
							...
							const specBinding = resolveDesignSpecBinding(ctx.cwd, n);
