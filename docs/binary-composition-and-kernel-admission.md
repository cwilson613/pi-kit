...
Mutation fencing and emergency recovery evidence on settlement-write failure remain Slice 3.5 work. Retry denial for mutating unknown-completion calls and migration of direct operator, extension, host-effect, and privileged internal compatibility paths remain later Slice-3 work. Reactive path grants and extension HostAction approval can only narrow the upstream lease decision.
