...
link:
    ...
    # Write a sourceable shell snippet that aliases omegon/om to this build.
    # Does NOT touch /usr/local/bin, ~/.local/bin, or any package-managed paths.
    ALIAS_FILE="$HOME/.omegon/dev-alias.sh"
    mkdir -p "$HOME/.omegon"
    printf '# Generated by just link — source this to use the dev build.\nalias omegon='\''%s'\''\nalias om='\''%s'\''\n' \
        "$BINARY" "$BINARY" > "$ALIAS_FILE"

    # Auto-source from shell profile if not already wired up
    ./scripts/wire-dev-alias.sh "$ALIAS_FILE"
