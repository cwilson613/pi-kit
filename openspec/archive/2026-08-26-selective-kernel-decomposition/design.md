...
Slice 7 closes release composition and deletion guards. Deterministic evidence is
versioned at `fixtures/release-closeout-evidence-v1.json` and validated by
`scripts/check_release_closeout.py`. It binds real maintenance success,
refusal, restart settlement, managed/dynamic cleanup, package surfaces, public
docs, and canonical snippets to executable tests. Offline signing evidence is
limited to the checked-in signed fixture and its positive/corruption tests; it
does not claim that a new release was signed during local closeout.
...
Slices 6.1, 6.3, and 6.4 now have one executable optional-domain proof matrix at `fixtures/optional-domain-proof-v1.toml`. The repository checker binds all nine optional domains to executable absence and degradation tests, per-domain architecture and composition classification, maintenance dependency exclusions, selected constitutional-source guards, and either applicable public feature documentation or an explicit no-public-change rationale. Slice 6.2 is an owner-neutral projection boundary rather than an optional domain. This evidence closes the Slice-6 isolation and documentation exit gates; Slice 7 supplies the separately packaged kernel and maintenance artifacts.
