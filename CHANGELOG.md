...
- Added a scenario-indexed operational kernel, signed core-component, and SDK-addon acceptance corpus. Twenty-five sentinel scenarios now separate scripted conformance from provider-backed execution and define boundary-specific promotion profiles across authority, prospective budgets, additive composition, generation lifecycle, surfaces, distribution trust, and cleanup. Structural release-policy tests reject invalid dimensions, duplicate scenario IDs, missing executors, and promotion profiles that omit corpus rows.
- Added one versioned session-activity projection for TUI, ACP, Web, IPC, CLI, and daemon surfaces. Durable authority revisions now fence stale, unversioned, conflicting, and cross-lineage activity observations; persistent edges recover missed terminal advice without corrupting a newer turn, while one-shot CLI output explicitly declares persistent busy reconciliation unsupported.
- Bound native-extension contribution-generation identities to their admitted source digests and made EventBus features plus direct polling handles consult the same published-digest fence before native RPC. Candidate digests remain non-callable until publication, and retained old-generation handles fail before process owner entry after the fence advances.
- Added explicit quiescent changed-generation publication for native extensions. Idle runtime or extension refresh stages admitted installed bytes, skips unchanged digests, and publishes compatible EventBus/native-RPC generations. One hidden candidate is retained per contribution; a newer candidate settles the superseded process before replacing it, active and queued session work prevents commit, turn completion does not publish implicitly, and successful commit atomically advances graph-derived aliases plus the shared RPC generation fence before bounded retirement of the old process. Widget and voice side-channel changes fail closed with a restart requirement, while `/runtime replace <name>` remains a same-generation admitted-snapshot respawn.
- Added generic signed-core qualification records that bind portable contracts, signed identity, and non-aliased boundary executors to one component and wire identity. Composition policy rejects missing, stale, aliased, and cross-component qualification evidence. Shared cleanup contracts and ACP diagnostics now preserve positively settled host resources separately from best-effort, unverified remote peers; transport closure cannot claim remote settlement without an acknowledgement protocol.
- Added a provider-neutral kernel runtime and provider-backed `omegon-kernel-host run` path. The reduced host admits an exact project inference route, persists shared turn and route authority before dispatch, consumes a bounded OpenAI-compatible stream, and records one terminal closure without importing product domains. Active cancellation and deadline timeout now settle the transport, persist typed terminal outcomes exactly once, and return stable bounded-task exit codes. Invalid task fields return a structured zero-turn rejection before route loading or authority creation. Task-capsule task files also reject unknown root and nested fields before runtime setup. Wall-clock and turn exhaustion are checked before the next route lease, with controlled below/exact/one-above coverage and a process-backed turn campaign that creates no request-two network activity. Continuation requests enforce admitted token budgets prospectively, preserve observed and admitted usage in structured exhaustion results, and create no over-budget route lease or network request. Shared invocation leases enforce atomic prospective tool budgets before lease creation and native RPC dispatch. The full-product bounded runner admits `[bounds].tool_budget`, shares it across parallel model calls, and reports observed calls plus typed exhaustion in its structured result.
- Added a deterministic bounded-turn conformance probe to the reduced kernel host. The real additive artifact ladder now requires byte-identical kernel-only and kernel-plus-codescan installations to complete the same single-request terminal turn within hard event and time bounds, with explicit scripted provenance and no extension process activation.
...
- Made direct installation fail closed unless an explicitly supplied, independently trusted external verifier authenticates the archive, canonical package manifest, Sigstore bundle, target, and composition before extraction. The authenticated staged maintenance companion now revalidates the exact extracted member tree before atomic activation; refusal preserves the active selector and removes installer-owned download and candidate staging.
- Made `omegon switch` capture maintenance authority from the active verified generation and use only that executable to authenticate canonical signed assets before and after extraction. Candidate code and metadata cannot grant activation authority, existing generation directories require byte and receipt equivalence with authenticated content, and failed switch work is removed without changing the single active generation selector.
- Added a CI-owned OCI evidence policy that requires signature, SBOM, provenance, composition identity, and an explicit composition class to bind one immutable image digest. Deterministic mutation fixtures reject missing or cross-digest evidence and false full-product inventory while explicitly recording that production publication and live registry verification were not performed.
...
- Added a deterministic release-closeout evidence contract and executable validator for maintenance success, refusal, durable settlement, cleanup, package surfaces, canonical snippets, and public migration/install guidance. Signing evidence remains limited to the checked-in Sigstore fixture and its tests. The maintenance companion now returns success for `--help` and `--version` and includes its embedded commit in version output, allowing linked launcher freshness checks to report `stale: no` for both executables.
...
- Stopped persisting skill-declared external paths as trusted directories. These declarations are now observable admission requests and require an authorized operator grant.

- Unified native extension, MCP process/HTTP, and executable manifest HTTP/script/OCI discovery under one metadata-only candidate inventory and contribution generation owner. Static discovery now captures stable identity and source digest without evaluating, spawning, connecting, resolving secrets, or publishing registrations; trust admission precedes transport probe; EventBus graph finalization remains the atomic publication boundary; and one owner records rollback, restart/quarantine, stale-generation, and cleanup outcomes. Existing JSON-RPC, HostAction, MCP resource/prompt, widget, secret-delivery, process-tree, OCI, and remote HTTP behavior remains transport-specific and wire-compatible.

- Made semantic surfaces owner-neutral for Slice 6.2. Concrete feature, managed-service, lifecycle, memory, settings, model, credential, filesystem, and process state is now adapted into immutable snapshots before projection; TUI, ACP, Web, IPC, CLI, and daemon action classifiers share one canonical action vocabulary; and Web/TUI/IPC operation views consume the same projection. Production source guards and cross-surface parity/golden tests enforce the boundary without changing command syntax, durable schemas, or frontend wire fields.

- Published Git as the final task-6.1 boot-captured managed service. One serial worker owns the discovered repository model, libgit2 access, and `omegon-git` Git/JJ process trees for core edits and commits, cleave, workspace control, and repository status. Typed absence and stale handles fail closed, host-approved workspace roots are explicit, and strict worker/process-set/writer cleanup joins complete process trees before retirement. Package and extension clones, updater and release probes, toolchain checks, TDD evidence reads, and test fixtures remain outside this boundary.

- Published context/compaction planning as an optional boot-captured managed service. Automatic pressure/overflow, feature-requested, manual, daemon, headless, bounded, Sentry, and ACP consumers now use one generation-tagged handle for deterministic eligibility, keep-window selection, and provider-payload formatting. Session context, semantic compaction facts, provider routing, conversation mutation, metrics, and events remain host-owned. Typed absence preserves ordinary context and unrelated turns without ambient lookup or direct planner fallback, while one strict worker provides cancellation accounting and bounded settlement.

- Completed the portable managed-memory campaign on macOS, Ubuntu, and Windows. Dedicated serial tests now cover persistence and reopen, mind-label isolation, retrieval and metadata, graph and episodes, JSONL recovery, vault convergence and containment, concurrency and replay, active cancellation, typed absence, context/status parity, generation transfer, stale handles, and strict file-handle settlement. A standalone source guard prevents production database, JSONL, or vault ownership outside the managed worker and explicit stopped-runtime migration.

- Cut memory tools, context, lifecycle ingestion, session-end persistence, provider-result writes, status projections, and embedding backfill over to boot-captured managed bindings or bounded managed compositions. Session-scoped payload-bound receipts preserve exact replay, context retains host-owned pins and budgets, provider work settles before managed shutdown, and status remains root-scoped with typed absence. Production consumers no longer open memory storage or use an ambient service-registry lookup.

- Moved project JSONL and configured Codex-vault synchronization behind the managed memory worker for Slice 6.1.9. The selected database and JSONL share one project root; empty-store startup import remains non-child and completes before readiness; explicit bounded import/export preserves transactional Lamport behavior and payload-bound replay. Explicit Codex integration now drives bounded contained snapshot reads, stable note/fact lineage, idempotent import and reinforcement, deterministic grouped materialization, cooperative cancellation, and synced atomic cross-platform publication. Static traversal, symlink escape, partial transient reads, stale operation reuse, global fallback, and unsettled filesystem handles fail closed.

- Added the Slice-6.1.9 boot-only managed memory owner. One bounded serial worker now serves explicit project and optional pre-existing global stores through owned version-1 status, fact, FTS/vector, graph, episode, and atomic mutation results. Payload-bound replay and entity-version conflicts remain typed; absent or uninitialized global files are never fabricated; queued cancellation skips execution; strict candidate rejection, panic, transfer, retirement, SQLite/WAL closure, and worker-before-writer settlement are covered. Existing tools, context, provider-result writers, JSONL, and vault consumers remain on compatibility paths pending their dedicated cutover checkpoints.

- Hardened project-memory persistence for managed-service preparation. Schema v8 adds payload-bound operation receipts and durable episode metadata, with governed migration from schemas v5-v7. SQLite compound mutations now commit mind, fact, status, vector metadata, edge, episode, JSONL, and replay effects atomically; retries return the original compact effect or reject operation-ID conflicts. SQLite and in-memory backends now preserve Lamport high-water marks, deterministic FTS tie ordering, JSONL batch rollback and idempotency, entity-version preconditions, and metadata parity. Existing v5/v6 `default` records still migrate to `legacy`, while post-v7 `default` records reconcile to `primensus`.

- Added revisioned managed OpenSpec mutations for Slice 6.1.8. Proposal, nested spec addition, task reconciliation and status, test registration, lifecycle transition, archive, abandon, and reopen now commit canonical artifacts plus one ledger snapshot through bounded checksummed journals. Archive and reopen recovery uses repository-relative contained paths, exact file/tree identities, ledger-last settlement, durable operation receipts, and deterministic roll-forward. Recovery rejects conflicting receipts before writes and quarantines malformed, oversized, unhealthy, path-tampered, or unknown transaction frontiers. Populated legacy OpenSpec roots remain selected when the primary root contains only metadata. Existing tool and surface consumers remain on compatibility paths pending the dedicated cutover checkpoint.

- Added crash-recoverable managed design mutations for Slice 6.1.8. Create, state and metadata updates, questions, research, decisions, links, implementation notes, branching, and implement-to-OpenSpec now stage canonical artifacts plus one lifecycle-ledger snapshot behind an exact repository revision and durable operation journal. Recovery preflights every resource and deterministically completes known partial commits; malformed, symlinked, conflicting, stale, rewrite-unsafe, or path-tampered state fails closed with versioned typed errors, while checksummed receipts make committed operation replay idempotent. Existing tool and surface consumers remain on compatibility paths pending the dedicated cutover checkpoint.

- Added the Slice-6.1.8 managed lifecycle read owner. One boot-only serial worker now owns the loaded opsx ledger plus design/OpenSpec repositories and returns owned snapshots, ready/blocked/frontier queries, stable-ID findings, health, and explicit safe-recovery results with deterministic repository revisions. Its strict task resource settles before its strict durable writer; candidate rollback, exact-generation transfer, stale handles, typed optional absence, conflicting OpenSpec roots, and unsafe primary-root recovery fail closed without exposing repositories or provider locks. Existing mutation paths and consumers remain unchanged pending the journal and cutover checkpoints.

- Added revision-checked lifecycle state persistence as the first Slice-6.1.8 hardening step. `omegon-opsx` now restores the complete last-persisted in-memory snapshot after any save failure, rejects stale whole-state writers instead of silently overwriting newer state, uses a cross-platform lock and durable unique temporary replacement, and loads legacy state without a revision as revision zero. Corrupt or future lifecycle state now disables the optional lifecycle feature locally during setup instead of retrying the same file and panicking.

- Published codescan as the first resource-bearing production managed service. One bounded serial worker now owns SQLite, indexing, HEAD freshness, and BM25 construction for `codebase_search`, `codebase_index`, and code-context requests. Boot-time startup failure degrades codescan locally, tools remain declared with typed unavailable evidence, mixed context requests continue unrelated kinds, and managed shutdown joins the worker before SQLite settlement.

- Completed the internal Slice-6.1.6 managed-service prerequisite without migrating a production domain. Renderer-neutral diagnostics now retain bounded DTO-only published/rejected owner history with actual active, draining, degraded, retired, resource-settlement, stop, force-stop, and rejection evidence. Interactive, daemon, headless, bounded, Sentry, ACP worker, cleave, and injected-runtime hosts explicitly await managed shutdown before releasing runtime ownership; unresolved strict and best-effort resources remain retained for later retry.

- Added graph-integrated synthetic managed-service publication for Slice 6.1.6. EventBus now prepares the candidate graph and legacy caches before one complete managed admission-table swap, then commits the prepared composition without suspension. Managed generations derive composition, owner, and contribution-generation identity from the graph, support multiple typed services, enforce implementation/resource/policy parity, preserve exact generations without cleanup, and retain every rejected owner across cancellation. The synchronous no-resource finalizer remains unchanged and fails closed when graph-managed generations are staged or active. Production-domain migration remains deferred.

- Extended the internal Slice-6.1.6 managed-service substrate with generation-owned resource controllers and retained cleanup orchestration. Managed generations now validate a frozen cleanup dependency DAG, grant active calls a separate bounded drain phase, clean resources in deterministic reverse-topological order against one non-resetting deadline, retain unresolved strict owners for retry, and prevent retirement before positive strict settlement. Graph-integrated publication, lifecycle projection, explicit host shutdown wiring, and production-domain migration remain deferred.

- Added the internal managed-service call substrate for Slice 6.1.6. Object-safe managed contracts now execute through generation-owned tasks behind one shared admission table, preserve accounting across caller cancellation and panic, close admission before retained cleanup, and require positive joined-cleanup evidence before retirement or ownership release. Production publication, lifecycle diagnostics, and domain migration remain deferred.
...
- Cut shared CLI, loop, and ACP repository-plan reads over to the boot-captured immutable work snapshot. Service absence now leaves session-local plan mutation and rendering available while repository plans/tasks are explicitly empty; production readers no longer fall back to ambient filesystem scans, and existing DTO and wire shapes remain unchanged.

- Published stateless behavior policy as an optional, generation-tagged in-process service and routed interactive, daemon/control, headless, bounded, Sentry, and ACP loop hosts through one boot-captured binding. Host-normalized immutable views preserve existing task-mode, phase, drift, progress/evidence, pressure, prose, and recovery-message behavior; absence holds session-owned policy state and omits only policy-derived pressure/meta retries while preserving ordinary turns and host-owned recovery. Missing optional dependencies now publish warning diagnostics without rejecting an otherwise valid contribution graph.

- Completed the Slice-5.6 compatibility-publication closeout. Valid legacy pairs now materialize one content-addressed compatibility base and establish mixed lineage before full-spine work; full and materialized-mixed sessions resume without and stop rewriting `.json`/`.meta.json`, while legacy and not-yet-materialized mixed sessions retain the pair only as a one-way importer. Maintenance now inspects and quarantines semantic sessions from catalog framing with legacy-pair fallback. Public session, command, migration, recovery, and TUI guidance plus canonical transcript/session-export snippets document exactness, compatibility, and repair boundaries without claiming an unimplemented rollback selector.

- Completed the Slice-5.5 recovery campaign so every frozen manifest row enters an exhaustive consumer-specific oracle, including projection, host-record, and mirror consumers that were previously skipped by the dispatcher; AC13 now reconstructs a real immutable chunk from a chunk-bearing mixed-lineage fixture, copied authority fixtures preserve restrictive permissions, and default projection output is checked for restricted-continuity leakage. The focused campaign passes on macOS, Ubuntu, and Windows within budget. Public documentation, canonical snippets, and compatibility-publication closeout were deferred to Slice 5.6.

- Completed the second bounded Slice-5.5 recovery campaign component: all frontend and evidence scenarios AC19-AC27 and AC37-AC45 now run deterministically with no pending manifest rows. IPC lag automatically publishes a same-connection full-state reconciliation before later deltas, ACP canonical completion bounds notification drain and releases skipped-terminal turns, Web/TUI/IPC idle reconciliation clears stale busy gates, and generation replacement discards retired overlays. Telemetry, audit, and journal probes now isolate malformed/partial evidence, stop semantic cursor advancement on corrupt semantic audit rows, preserve nonsemantic rows, and label authority-backed journal replay failures `semantic_source_unavailable` without blocking runtime or replacement.

- Added the first bounded Slice-5.5 recovery campaign component: a checked 54-ID manifest, deterministic sandbox and injection barriers, and implemented exact/projection/host-record/mirror scenarios. Corrupt derived chunks now quarantine and rebuild under the projector lock, authority-backed sessions cannot downgrade through a missing catalog, observation-ledger absence honors durable existence evidence, and semantic success followed by rollback-mirror failure returns typed partial publication. Frontend and diagnostic/evidence scenarios remain explicitly partitioned for the next component; no ACP, IPC, journal, audit, schema, or dual-write behavior changed.

- Completed the bounded Slice-5.4 consumer migration under the frozen plural-authority and exact-frontier contract. Full resume no longer reads rollback-mirror conversation bytes, catalog lineage/availability is checked against strict replay, migrated adapters are source-guarded against direct projection-storage access, and native `/transcript` plus `/session-export` help reflects the implemented split. Legacy `.json`/`.meta.json` dual-write remains active through Slice 5.6 without admitting old-writer downgrade. Remaining Slice 5.5 frontend/diagnostic campaigns and Slice 5.6 applicable public session/command/migration/recovery documentation and dual-write closeout remain explicitly deferred; no canonical snippet changed.

- Migrated the bounded Slice-5.4 Markdown journal and runtime diagnostic audit consumers to validated semantic replay. Journal entries now append stable source provenance without rewriting legacy narrative, derive committed counts/prompt/outcome/routes while keeping usage and workspace state observational, label sessionless fallback, and deduplicate exact source frontiers. Semantic runtime audit rows use source-event identity and a strict output-before-cursor v2 consumer checkpoint across restart/replacement, while permission, progress, UI, and policy observations remain on their owning event streams and advisory failures remain non-authoritative.

- Migrated bounded Slice-5.4 turn telemetry checkpoints to a strict, durable version-1 stream bound to dynamic session, authority stream, host generation, canonical turn closure, and semantic source frontier. Semantic counters now reduce from authority while provider usage and context estimates remain labeled observations; replacement fences stale events, malformed tails fail visibly without mutating authority, legacy checkpoint JSONL remains compatibility-readable, and checkpoint failure cannot block terminal commitment or operator progress.

- Migrated bounded Slice-5.4 Web and IPC consumers to the atomically published host-session binding. Web startup, reconnect, replacement, historical reads, and lag recovery now reconcile validated frontend/transcript projections with explicit lineage, exactness, availability, frontier, queue, turn, and context disclosure while fencing old-session overlays; IPC v1 adds compatible optional dynamic session-generation/projection fields and authoritative queue state without adding transcript content.

- Migrated the bounded Slice-5.4 transcript, TUI, and ACP consumers to validated semantic session projections. `/transcript` now exports exact committed full-lineage content, explicitly permits only a labeled exact suffix for mixed lineage, and reports legacy semantic history unavailable; `/session-export` retains presentation/evidence export with provenance and ephemeral-overlay disclosure. TUI startup and replacement atomically seed committed and durable partial/abandoned frontend state behind a generation fence without treating projection lag as runtime activity, while ACP publishes replacement identity before replaying validated user/assistant history, omits historical tool activity, and exposes lineage/frontier status.

- Cut authority-backed full and mixed sessions over to synchronous exact-frontier provider context. Dispatch now uses ordered semantic prompt, assistant, tool-result, and applied-compaction state rather than compatibility provider history; mixed sessions freeze one labeled legacy base, request preparation must byte-match the dispatched context and generated system/tool inputs, restricted continuity remains lineage-authorized, and legacy/sessionless execution retains the typed compatibility path.

- Added the bounded Slice-5.4 host persistence component: strict versioned host-state checkpoints, durable append-only operator-observation ledgers, and joined session catalog records now bind to semantic identity/frontiers; catalog-backed sessions list and resume without a compatibility snapshot, replacement validates the complete target store set before publication, and unchanged schema-v1 `.json`/`.meta.json` rollback mirrors continue dual-writing without overriding full-spine state.

- Completed shadow-only Slice 5.3: one immutable semantic session read model derives ordered lifecycle, request/attempt, committed and partial message, tool, compaction, lineage, frontier, and availability evidence from validated strict replay. Frozen schema-v1 provider-history, transcript, frontend-snapshot, compaction-checkpoint, chunk, manifest, and envelope DTOs enforce canonical deterministic bytes, bounded outputs, exact mixed-suffix/full-session claims, legacy availability-only output, and exclusion of restricted continuity metadata and bytes. Each authority-backed supervisor now owns one capacity-one, dirty-bit projection worker for its session lifetime; post-durability hints coalesce on the frozen cadence, terminal/startup/shutdown boundaries flush immediately, and every run captures the latest stable frontier before independently publishing all four projectors through cursor v1. Typed failures preserve prior outputs and do not block authority appends, terminal commitment, replacement, or operator progress. Atomic replacement fences and transfers owned cleanup of the old worker, sessionless hosts create none, and current provider, conversation, transcript, frontend, snapshot, and compaction consumers remain unchanged for Slice 5.4. No command, configuration, public-site, or canonical-snippet behavior changed.

- Completed the Slice-5.2 authority compatibility boundary: full-spine request capture now requires event-backed prompt, assistant, tool-result, generated-source, and compaction provenance without emitting a legacy transcript fallback; strict replay and blob authorization, response-attempt recovery, reducer/cache v5 indexes and fixtures, compaction facts/recovery, generic output-before-cursor storage, and idle-only atomic host session replacement are active. Already-durable legacy facts remain readable, sessionless execution remains route-only, and concrete projections, consumer migration/recovery, and final documentation remain Slices 5.3-5.6. This internal boundary changes no command, configuration, public-site, or canonical-snippet behavior.

- Added idle-only atomic host session replacement for interactive `/resume`, `/new`, `/context clear`, ACP load/new/control, and daemon control. One owner now validates authority recovery, blobs, replay frontier, execution binding, projection identity, and prompt/invocation quiescence before publishing session identity, compatibility conversation, supervisor, resume metadata, and host generation together; failures preserve the original usable session and ambiguous queued prompts are rejected rather than moved. `/context clear` starts a fresh authority lineage and sessionless compatibility resets remain unchanged.

- Added the Slice-5.2 session compaction authority: exact event-backed input and retained manifests, turn-owned unchanged route-lease joins, idle session-scoped inline route evidence, provider-Done-only summary commits, durable exactly-once apply before compatibility mutation, explicit failure abandonment, v5 replay indexes, and compaction-first deterministic recovery. Sessionless compaction retains route-only behavior without semantic facts; concrete provider-history/transcript/frontend/compaction-checkpoint projectors remain deferred to Slice 5.3.

- Added Slice-5.2 authority-backed provider retry and replay components: retries append strict `model.response_attempt_failed` v1 facts before retry, contiguous response-attempt lineage includes zero-content tool-call commits, and crash recovery derives deterministic remaining invocation/request/step/turn closure facts entirely from durable prefixes. Read-only replay strictly reduces stable authority prefixes to exact immutable frontiers, classifies legacy-only/mixed/full-spine lineages, validates every referenced blob, gates restricted continuity reads by request and serving lineage, and exposes incomplete or abandoned state without appending recovery facts. Snapshot/reducer v5 records the full-spine boundary and complete owning event-family indexes, and a checked canonical JSONL/blob corpus freezes compatibility and corruption vectors. Sessionless retry evidence remains in the existing side log; concrete projections and consumer migration/recovery remain deferred.

- Activated the complete Slice-5.1 semantic spine for authority-backed sessions: exact provider-facing context and schema snapshots, request/route joins, response-attempt-scoped 64 KiB/50 ms assistant chunks before broadcast, provider-Done-only message commits, restricted continuity, provider-ordered tool calls and terminal results with invocation linkage, truthful request outcomes, and exactly-once step terminals. Version-4 authority recovery and every host abnormal turn closure now classify unresolved dispatched/acknowledged invocations, close an open request with its response-attempt identity, append deterministic UUIDv5 `step.abandoned`, and only then commit the narrowed turn outcome after owned cleanup. Interactive cancellation drains the same pinned execution until return or bounded supervisor abort, then joins its turn-scoped projection relay before terminal commitment; detached native HTTP completion is externally unverified and cannot mutate or project into this or a later turn, while process-backed tools retain tree-scoped cleanup. Complete authority is required; partial scopes fail closed and sessionless hosts retain route-only evidence without fabricated semantic facts. Replay compatibility matrices, derived transcripts/provider history/frontend/compaction state, legacy storage migration, and consumer recovery remain Slice 5.2-5.5 work.

- Added versioned renderer-neutral runtime contribution contracts for stable contribution/capability identity, distinct composition and owner generations, owner tier, requested trust and confinement, canonical and alias bindings, dependencies, conflicts, replacements, groups, platform requirements, effects, lifecycle and transition policy, execution semantics, surface support, and deterministic diagnostics. JSON fixtures freeze declaration, generation, and diagnostic wire shapes without changing legacy composition or dispatch authority.

- Added a pure deterministic candidate contribution-graph builder with complete all-error diagnostics for duplicate ownership, ambiguous bindings, replacement and dependency cycles, missing requirements, conflicts, protocol/platform incompatibility, dangling aliases/groups, and undeclared requested or observed effects. Invalid candidates publish no partial graph; valid candidates produce stable ownership indexes, negotiated protocols, dependency edges, and activation waves without changing current runtime dispatch.

- Routed static EventBus publication through staged declaration adaptation, candidate-graph validation, activation-plan/parity readiness checks, and graph-derived legacy caches. Tool, command, alias, and internal-binding collisions now fail closed independent of registration order; rejected hot additions preserve the previous accepted dispatch, and arbitrary external display names or malformed vocabulary are handled without startup panics.

- Added a versioned non-executable dynamic-contribution preflight contract that binds identity, immutable source digest, source kind, protocol/dependencies, requested trust/confinement, and bounded probe requirements. Separate host-produced admission evidence is source-bound and validates explicit kernel-release/operator trust or complete broker-only OS/OCI confinement without treating manifest requests, installation, enablement, maintenance admission, or trusted-directory state as grants.

- Required explicit `permissions.trustedContributionCode` admission before native or OCI extension secret preflight, spawn, and protocol probing. Admission is bound to the complete snapshotted source-tree digest and rechecked at initial spawn and respawn; current OCI execution is not misreported as verified confinement.

- Extended trusted-code admission to Pkl evaluation, Armory context/script/OCI paths, legacy HTTP plugins, plugin/project MCP, and ACP-submitted MCP. Production constructors and deferred execution boundaries require source-bound permits, and untrusted configuration is rejected before secret expansion, spawn, or network connection.

- Added versioned contribution lifecycle and owned-resource records carrying composition/owner generation, last completed boundary, bounded coded reason, restart and heartbeat evidence, cleanup assurance, and cleanup state. Contract validation rejects false strict-cleanup and remote-service ownership claims.
...
- Bounded MCP transport and capability discovery by one per-server readiness deadline and added explicit `RunningService` supervisors. Startup, ACP rejection, and runtime shutdown now close MCP candidates with bounded cleanup reporting instead of relying on asynchronous drop cancellation.

- Added bounded extension restart/backoff policy. Repeated transport failures consume a generation-local restart budget and quarantine the extension after capped exponential backoff instead of spawning indefinitely.

- Made Armory context, script, and OCI process timeout/cancellation tree-scoped and reap-complete. Script paths are now confined to normal relative paths inside the admitted plugin snapshot.
...
- Reconciled dynamic-contribution authoring and operator guidance with the implemented trust and lifecycle boundaries. The canonical host guide now documents stable extension/plugin/MCP identities, source-bound admission, trusted execution versus verified confinement, readiness, restart quarantine, cleanup assurance, and composition diagnostics; the injected authoring reference now uses the current `execute_tool` RPC and guarded-copy installation model.

- Routed model-tool admission through one kernel invocation service that resolves the accepted contribution graph, combines RBAC, layered permission policy, and operator approval monotonically, and issues generation- and owner-bound execution leases. Dispatch now rejects stale, mismatched, reused, unknown, or incompletely declared authority before owner execution and closes in-memory leases exactly once; durable invocation recovery and remaining privileged compatibility paths are unchanged pending later slices.

- Replaced model-tool name-derived execution authority with validated capability metadata for principal eligibility, required effects, bounded timeout class, parallel safety, retry/idempotency/deduplication, transaction behavior, and transition policy. RBAC now derives from declared effects, arbitrary declared parallel-safe tools can run under the kernel concurrency ceiling without a source allowlist, caller timeouts cannot widen declaration limits, and conservative adapters fail closed without claiming precise external authority.

- Persisted `invocation.prepared` before issuing authority-backed model-tool leases and `invocation.dispatched` before host or local owner handoff. Interactive, ACP, daemon, and bounded turns now propagate stable invocation, visible call, optional owner-enforced deduplication, session, and turn identities through one shared authority writer and owner context; write failures prevent handoff, while acknowledgement, terminal settlement, and unknown-completion recovery remain deferred.

- Persisted owner acknowledgement and typed terminal settlement for authority-backed model-tool calls before completion publication and lease closure. Local, host-delegated, extension, and MCP execution now share one acknowledgement control; ambiguous external transport loss is durably classified as unknown instead of replayed, and recovery classifies every unsettled dispatched or acknowledged call as unknown while preserving prepared calls that never reached an owner.

- Added durable mutation domains and fence keys to mutating execution declarations. Post-dispatch acknowledgement or settlement durability failures now write independent append-only recovery evidence and deny later matching mutations before preparation; malformed evidence and emergency-writer failure fail closed, while ordinary completion and unaudited fence removal remain prohibited.

- Denied replay of stable call identities whose original mutating invocation has unknown completion without persisted idempotency or exact owner-enforced deduplication. The check spans prior turns, uses the original contract rather than replacement metadata, treats legacy unknown records conservatively, and prevents ambiguous ACP host-write failures from falling back to a second local mutation.

- Generalized invocation admission and generation revalidation beyond model tools. Graph-registered TUI, CLI, ACP, Web, and IPC feature commands now execute under explicit operator leases and owner-declared surfaces; model-loop path grants, automatic memory ingestion, and host-mediated persona/tone switches use internal leases; managed-delegation tools admit explicit service principals and Web/Daemon surfaces instead of dispatching directly. Memory mutation and delegate-result tools now declare state-changing effects, operator context-pack reads use a typed read-only service instead of tool dispatch, extension-provided voice stop uses the promoted turn's declared TUI service lease, and daemon vox polling uses the declared Service/Daemon tool lease. Arbitrary ACP extension methods dispatch through one conservative extension-owned Operator/ACP transport lease on the worker EventBus instead of raw polling handles. Lease-less imperative extension HostActions fail closed, approval no longer widens independent policy/trust gates, and declarative native/MCP HostActions require a live parent guard with effect containment and exactly-once child identity.

- Reconciled developer and public invocation guidance with the implemented permission, lease, retry, and recovery boundaries. Documentation now distinguishes the permission layer's no-rule default from fail-closed capability admission, provider-request retry from post-dispatch invocation replay, and acknowledgement/unknown/terminal durability fencing from the reserved audit-settlement evidence vocabulary; it also records that no automatic safe-replay scheduler or operator command currently reconciles historical unknown invocations or clears mutation fences.

- Added a validated provider-contribution registry binding stable owner/generation identity, runtime inventory authority, authentication class, executable tool-schema support, bridge-factory identity, modality/capability evidence requirements, and directed model-family fallback compatibility. Provider parsing, schema lookup, family fallback, and bridge construction now consume the registry; incomplete or ambiguous candidates fail with deterministic diagnostics, while cross-host route authority and durable request leases remain assigned to the next provider-routing slice.

- Added one contribution-backed provider route service and pre-dispatch route leases across interactive, ACP, daemon, child, bounded, compaction, smoke, and lightweight inference paths. Session requests durably bind selected and serving provider/model identities, schema and credential classes, fallback reason, contribution generation, and route policy to the active turn; sessionless work records the same evidence under an explicit durable step identity, while stale generations, undeclared fallbacks, partial session scopes, and failed lease writes prevent provider dispatch.

- Added one release-coupled loop driver over required session, leased-route, context-assembly, and privileged-invocation contracts. Every compiled runtime host now enters through the same typed turn boundary; stale or incomplete session authority and missing or contradictory serving-route identity fail before execution, and authority-backed hosts consume one driver-produced terminal proposal after cleanup instead of independently reclassifying ordinary completion and failure.

- Removed concrete provider, tool, memory, lifecycle, permission, plan, context, and frontend implementations from production loop policy. The release-coupled driver now captures their host configuration through one opaque binding, provider routing consumes a route-port policy snapshot instead of calling back into `LoopConfig`, and architecture guards reject direct authority bypasses or route/invocation adapter cycles; driver replacement/quiescence and complete semantic message/step persistence remain deferred.

- Restricted loop-driver and provider-route-service replacement to one atomic boot or explicitly committed quiescent session binding. Mid-turn requests remain in-memory pending across ordinary turn closure and later turn start until a deliberate caller commits them; durable migration precedes publication, unresolved or unknown invocations block migration, concurrent start cannot capture a mixed pair, and resumed boot binding appends no synthetic history. All durable hosts use the session-owned capture, while sessionless execution uses the immutable boot binding.

- Added OpenRouter's `stealth/ox-alpha` preview route to the embedded model inventory and curated model menu with its published 1,048,576-token input context, 131,072-token output limit, multimodal input, reasoning/tool support, and stealth-provider retention boundary.
...
- Added release-distribution support for packaging and launch-validating `omegon` with `omegon-maintain` across source, linked-development, direct-install, and platform-archive paths, with deterministic composition policy for candidate Homebrew, Nix, and OCI layouts. Homebrew, Nix, and OCI remain deferred rather than public release lanes. Script installation, pairwise self-update, and version switching now publish immutable complete generations and atomically activate the executable pair with its install receipt through one shared `current` link, retaining the prior generation for interruption safety and rollback. Canonical per-target package manifests and tag-bound offline Sigstore bundle-v0.3 evidence omit timestamp-authority material; `omegon-maintain release verify` validates those explicit operands offline against compiled identity and trust-root policy. Public installation, security, and recovery guidance now consumes unique canonical snippets and documents bounded diagnosis, denial/quarantine, stale-record pruning, audit evidence, and offline package verification.
...
- Added provider-neutral model browsing over current runtime inventory evidence. `/model` now opens a curated provider-grouped shortlist and exposes live discovered routes when available, including Ollama Cloud `/api/tags` results; operators can persist global favorites with `Space`, select concrete routes with `Enter`, and ACP consumes the same shared projection instead of maintaining a separate static/probed catalog. This is not an exhaustive current provider or model inventory.

- Completed the Slice 4 public documentation closeout for provider contributions and route leases. Provider, routing, model-control, authentication, session, recovery, migration, ecosystem, FAQ, README, and related architecture guidance now distinguishes declared inventory from executable routes, bounds fallback as directed and non-transitive, records Antigravity as currently non-executable and headless Anthropic subscription use as warning-only, and avoids claiming lease-history commands, dispatch-gating inventory diagnostics, exhaustive current inventories, or precise credential stores where only authentication-class evidence exists. Commands, configuration syntax, and canonical site snippets are unchanged.
...
- Guarded local extension installation, enable, disable, configuration, runtime stability-state persistence, and removal with the canonical user-extension mutation lock. Local installs now publish immutable copied bundles instead of external symlinks; nested state and config writes are descriptor-confined and generation-identity checked, while legacy development symlinks can be removed without following or deleting their external targets.
- Enforced maintenance contribution exclusion for canonical user extensions during normal runtime composition and direct TDD extension execution. Startup now excludes exact denied basenames, fails malformed scope authority closed, snapshots admitted bundles before executing native content, retains the shared scope lock through final EventBus publication, and reuses the immutable generation for transport respawn.
...
- Replaced body-text sniffing in operator-surface routing with command provenance. Command output previously reached the TUI as an untyped `SystemNotification`, so the renderer had to guess the producer by matching body prefixes (`"Usage\n\n"`, `"## Skills\n"`). Rewording any formatter silently demoted its surface to a transcript dump — the exact regression that hid the `/usage` modal. Output now travels as `AgentEvent::CommandSurface { command, body }` and every surface routes on the command. The compiler enumerates consumers, so a new surface can no longer be added without deciding how each one renders it.
- Fixed a `HOME`-dependent race in the ACP config-options test. It resolved the active profile twice — once inside `build_config_options` and once to compute its expectation — so a concurrent test mutating `HOME` between the two reads made them disagree. It now holds the shared auth env lock across both reads.
- Fixed a leaked `HOME` environment variable in the init-menu skills test. It repointed process-global home resolution at a tempdir for every subsequent test, causing an unrelated ACP profile assertion to fail under interleaving. `HOME` mutation now uses an RAII guard serialized on the shared auth env lock, matching the convention already used in `auth.rs` and `route.rs`.
- Fixed `/usage refresh`, `/usage --refresh`, and `/limits -r` falling back to a plaintext command panel. The metric-modal router matched `panel.source` against the whole command line, so any argument defeated the match and silently demoted the documented refresh forms to the generic renderer. The router now dispatches on the command token.
- Rendered capacity reset times as operator-meaningful values. `/limits` and `/usage` previously printed `resets at unix 1799999999`, which requires the operator to do epoch arithmetic to learn anything. Windows now report time-to-reset plus local wall-clock time (`resets in 2h 14m (Tue 14:03 local)`), and an elapsed window reads `reset due`. Clock precision scales with horizon — bare time inside a day, weekday inside a week, calendar date at 7 days or beyond — because a Codex weekly window is exactly 10080 minutes out, where a bare weekday collides with today, and monthly buckets (GitHub Copilot premium requests / AI credits, reset on the 1st) run further still.
...
- Routed lifecycle/OpenSpec consumers through the managed lifecycle owner. Tools, context, setup snapshots, ACP, TUI, Web, IPC, workflow, Sentry hooks, work aggregation, project rules, prompts, startup status, session/check-in projections, and Codex export now use boot-captured handles or immutable managed observations instead of reconstructing repository authority. Work aggregation retains immutable boot publication, one-shot CLI checks shut down their managed owner, and source guards keep direct lifecycle scanners and repositories test-only. Lifecycle tools remain declared with typed unavailability when the optional managed owner cannot start. A serialized macOS, Ubuntu, and Windows campaign covers repository races, crash recovery, compatibility, strict cleanup, optional absence, and cross-surface parity. External Markdown authoring, append-only TDD evidence, and stopped-runtime migration remain separate authorities.
...
- Extracted native `AgentEvent` projection from the monolithic TUI event-loop module into `tui/agent_events.rs`. Channel receive, scheduling, and redraw cadence remain in `run_tui`, while tool lifecycle, streaming-message, permission, workbench, retry, usage, and notification state transitions now cross one explicit `AgentEvent -> App` adapter boundary.
- Extracted the native semantic `UiAction` adapter from the monolithic Ratatui module into `tui/ui_actions.rs`. Shared prompt, composer, conversation-segment, copy, UI-preset, permission, and operator-wait actions now cross one explicit adapter boundary, while rendering, layout, and frontend-local navigation remain in their existing Ratatui owners.
- Extracted canonical slash-command parsing and the native TUI slash-dispatch surface from the monolithic Ratatui adapter into `tui/slash_commands.rs`. The new module owns command normalization and maps canonical commands onto TUI state/coordinator actions, while `tui/mod.rs` retains event-loop invocation and rendering; existing CLI, ACP, and control-runtime consumers continue through the same crate-visible canonical parser re-export.
- Extracted native Auspex discovery, compatibility probing, attach-payload construction, transport-security derivation, and process/browser handoff from the monolithic Ratatui adapter into `tui/auspex.rs`. Slash routing and operator-facing status presentation remain in `tui/mod.rs`, while the process boundary now has focused protocol-version, handoff-fallback, and payload-security tests.
...
- Installed the docs site's declared npm dependencies before its content/build test in a least-privilege Linux/Windows CI matrix, made the test's Windows npm build invocation use an explicit command processor with a fixed payload, and made snippet generation deterministic and fail fast when required documentation snippets are absent.
...
- The docs-site build now fails loudly when a repo-root document (`CHANGELOG.md`, `TERMS.md`, `PRIVACY.md`) cannot be read, instead of silently shipping "Not available during this build" placeholder pages to production.
...
- Om now renders a dedicated one-line essential status projection instead of the legacy session telemetry row: attention/current state wins, followed by model, workspace/branch, and context as width permits; Active and Full retain the richer operational status renderer.
- Added deterministic replay coverage for the Om activity-to-history handoff: the same canonical conversation state is projected before and after tool completion, proving live activity disappears only when one durable episode outcome is available and canonical evidence remains unchanged.
- Updated the session-reset TUI regression to assert semantic instrument activity rather than legacy rendering copy, preserving coverage across the new Om/Full presentation split.
- IPC/WebSocket snapshots now expose active delegate/cleave episodes as stable semantic DTOs with episode ID, kind, state, bounded outcome, child evidence references, and completed/total progress; the additive field defaults empty for compatibility.
- IPC `get_state` snapshots now carry an optional, backward-compatible semantic presentation snapshot from shared settings, including current Om/Active/Full level, density policy, and supported-level capabilities; WebSocket consumers inherit the same typed state contract.
- Added a versioned semantic presentation surface envelope for remote clients, advertising the current named level, transcript/live/telemetry density, subordinate surface visibility, and supported Om/Active/Full capabilities without inferring policy from dashboard booleans.
- Added deterministic episode replay fixtures that prove completion and failure atomically replace live activity with one durable outcome at the next revision, preserve episode identity, and never expose a disappearance or durable-duplication state.
- Active workflow child summaries are now explicitly truncated to terminal width before rendering, preventing result text from wrapping and consuming multiple stage rows.
- Om attention escalation is local and sticky: blocked/waiting/pending-approval Workbench states remain visible while routine work stays hidden, and the global Om presentation level is unchanged.
- Active delegate/cleave activity now renders as a bounded parent workflow tree with authoritative operation identity, completed/total progress, text-plus-glyph child states, failure/running-first stage selection, and deterministic overflow summarization.
- Om live activity now has an explicit one-row layout budget regardless of tool detail preference, preventing a single running tool from expanding quiet mode into a four-row diagnostic card; Active retains bounded workflow detail.
- Workbench projection is now presentation-aware: Om suppresses routine plan/workspace/recent-work chrome unless an approval, blocker, reconciliation issue, or pending decision requires attention; Active retains the current execution slice; Full preserves the complete operational surface.
...
- `/transcript`, native scrollback transcript output, Markdown transcript files, and `/copy session` now consume the semantic export projection explicitly, so their default archival content is independent of whether the TUI is currently Om, Active, or Full.
...
- Added cleave inventory-generation pinning: run launch resolves every unpinned child route against one shared inventory generation before orchestration, records per-child route provenance in the plan, and rejects launch if the generation changes during resolution rather than silently mixing routing estates.
- Persisted delegate route provenance in child task state and semantic operation projections, exposing selected model, requested grade, parent ceiling, inventory generation, route source, and fallback reason consistently to TUI/web/ACP consumers.
- Routed direct delegate selection through the shared profile-aware `subagent_route` resolver, so scout/patch/verify requests now use capability-specific grades, respect parent-grade ceilings, preserve explicit model authority, and capture generation/source/fallback decisions without creating a second routing path.
- Routed inherited delegate model selection through the shared inference runtime: explicit child model pins remain authoritative, while unpinned children record inventory shadow evidence and may use parity-qualified inventory preference without introducing delegate-local inventory state.
- Began subagent integration by injecting the shared inference runtime into delegate feature construction, giving delegate route selection access to the same inventory, manifest generations, parity evidence, and authority policy as the parent runtime without creating a delegate-local store.
- Added a disabled-by-default inventory authority experiment: `OMEGON_INFERENCE_ROUTE_POLICY=inventory_prefer` can prefer only current-generation, parity-qualified inventory offerings that map to compiled provider bridges, with authoritative routing retained as immediate bridge-construction fallback.
- Hardened route-shadow quality with normalized exact/conceptual/provider/divergence classifications, provider and exact-pin constraint parity, generation-local readiness metrics requiring at least 20 observations and 95% normalized parity, bounded safe divergence details, and direct classification/readiness regression coverage.
...
- Added the dynamic inference inventory foundation: provider integrations, endpoint deployments, and model offerings now have typed runtime identities, field-level source/evidence provenance, layered deterministic merging, validated atomic generation refresh with last-known-good retention, explicit ungraded-route policy, modality/interface compatibility filtering, and an adapter that projects the embedded model registry as bootstrap inventory.
